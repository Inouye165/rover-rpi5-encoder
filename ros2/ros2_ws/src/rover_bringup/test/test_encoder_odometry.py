...
class TestRoverEncoderOdometryNode(unittest.TestCase):
    ...
    def test_node_initialization(self):
        ...
        self.assertEqual(self.node.get_parameter('track_width_m').get_parameter_value().double_value, 0.382)
        ...
        self.assertEqual(self.node.kinematics.track_width_m, 0.382)

    def test_runtime_config_and_server_defaults_match_code_default(self):
        """Verify that server.js, app.js, and launch/node default parameter track_width_m is 0.382."""
        ...
        if os.path.exists(server_js):
            with open(server_js, 'r', encoding='utf-8') as f:
                content = f.read()
            self.assertIn('let TRACK_WIDTH = 0.382;', content)
            self.assertIn('effectiveTrackWidth: 0.382', content)

        # Check node default declared parameter
        param_val = self.node.get_parameter('track_width_m').get_parameter_value().double_value
        self.assertEqual(param_val, 0.382, f"Runtime launch/node default track_width_m must be 0.382, got {param_val}")
