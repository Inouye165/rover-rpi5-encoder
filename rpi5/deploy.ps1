    ...
    Get-Content ".env" | ForEach-Object {
        ...
        if ($line -and -not $line.StartsWith("#")) {
            ...
            if ($parts.Length -eq 2) {
                $key = $parts[0].Trim()
                $val = $parts[1].Trim()
                if ($key -eq "RPI_IP" -or $key -eq "ROVER_PI_HOST") { $IP = $val }
                elseif ($key -eq "RPI_USER" -or $key -eq "ROVER_PI_USER") { $USER = $val }
                elseif ($key -eq "RPI_PASSWORD" -or $key -eq "ROVER_PI_PASSWORD") { $PASSWORD = $val }
            }
        }
    }
}
